...
对于同一输入，其 ID 与 HF 从头编码的结果相同；在解码选项相同的前提下，两个
解码器返回的文本也相同。默认选项本身并不相同：这里的 `decode` 默认跳过特殊
token（`skip_special_tokens=True`），而 HF 的解码器默认保留它们，因此含特殊
token 的 id 在默认设置下会解码出不同文本，除非把其中一侧的设置传给另一侧。
两侧传入相同取值时，结果逐字节相同。
...
那个缓存；已经持有该别名的缓存会被重新校验，而不是被覆盖。若已装的目录仍
逐项认证为同一个 bundle——每个声明路径、字节数与 SHA-256 都相符，除 toktier
自己写下的校验标记外没有未声明的文件——这次 import 就是幂等的，直接返回已在
那里的目录；把上面的命令
跑两遍，或在刚由 `fetch` 写入同样字节的联网主机上 import，走的都是这一支。
若目录里是别的内容，import 会停下来并点名其中一个文件，而不是覆盖别人可能
正在用的字节：树中只要存在 bundle 未声明的条目（任意深度），点名的就是它；
只有不含这类条目的树，才会去找第一个缺失或不相符的已声明文件。无论哪一支，
bundle 本身都是验证通过的。
...
`automatic_gpu_candidate: true` 与 `jit_toolchain_satisfied: false`，随后按当前
生效的策略作答：在默认的 `SUPPORTED` 策略下为 `automatic_gpu_eligible: true`
与 `automatic_effective_backend: gpu`——该策略会运行这一未经评审的组合，并把
它标为 `supported_untested`；在 `CERTIFIED` 下，同一台机器则读到 `false` 与
`fast_cpu`。两种情形下，这个结论都与 `toktier gpu compile` 以及下一次请求所走
的路一致，而且无需先运行它们。
...
工件**（按精确工件继承覆盖 12 个 family），以及**注册表 215 条**（214 个
...
随包注册表共 215 条：上述 214 个 sibling，外加 `moonshotai/Kimi-K3`
自身——这样按名解析 canonical 仓库时，报告的 evidence 仓库就是它自己，
而不是某个字节完全相同的 sibling。其中 208 条会映射到当前 wheel 随附的
canonical 工件。其余 7 个是 WordPiece 条目，对应 canonical 工件尚未打包，
因而使用 HF。13 个源码级 `kimi_k3` 条目已在这 208 个之内：其 canonical
...
- [`docs/releases/v0.2.9.md`](docs/releases/v0.2.9.md) — 本版本发布说明（英文）。
