...
- **CPU 与 GPU 两条快路径。** 在归档基准中，GPU 路径处理一个全新的
  400 万字符请求（约 78.6 万 token）仅需 **3.88 ms**；对 419 万字符会话
  追加 256 个字符的原生有界 CPU repair 操作仅需 **1.68 ms**。该数字衡量
  repair 本身，不包含将完整历史 token 序列物化为 Python tuple 的成本；
  原生 Rust serving 集成可以将会话状态保留在 Rust 中，并只消费修复后的
  token suffix，从而避免完整历史序列的物化。
