...
impl PrecompiledCharsmap {
    ...
    pub(crate) fn normalize_into(&self, input: &str, out: &mut String) {
        ...
        let mut i = 0usize;
        while i < bytes.len() {
            // Hop to the next attention byte (non-ASCII, control, DEL).
            //
            // Upstream used unstable portable_simd here. TokTier keeps the
            // identical predicate in stable Rust so the core wheel builds on
            // the repository's supported toolchain without RUSTC_BOOTSTRAP.
            let mut j = i;
...
impl SentencePieceBPE {
    ...
    // The fragment-split trio below is retained with the vendored
    // SentencePiece surface; no shipped configuration reaches it, so it
    // carries explicit dead_code allowances.
    #[allow(dead_code)]
...
impl SentencePieceBPE {
    ...
    /// Walk 32-byte blocks whose mark-candidate bitmask is drained bit
    ...
    fn encode_units_impl<const RAW: bool, F: FnMut(&[TokenId])>(
        ...
        while block < bytes.len() {
            let mut mask = 0u32;
            let end = (block + 32).min(bytes.len());
            for (i, &b) in bytes[block..end].iter().enumerate() {
