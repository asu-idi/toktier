...
/// verbatim.
enum Piece<'a> {
    // The source byte offset travels with every segment for verify-heavy
    // consumers; the shipped encode paths do not read it.
    #[allow(dead_code)]
    Segment(&'a [u8], usize),
    Added(TokenId),
}
