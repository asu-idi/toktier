...
| `BackendUnavailable` | `BACKEND_UNAVAILABLE` | A required backend cannot be used under a policy that demands it (`REQUIRE_ACCELERATED`). `missing` says which precondition failed: the backend module name when the package/extra is not importable, `"cuda_device"` when a performed device probe found no usable device, or `"device_probe"` when device enumeration was never performed because the requesting path adopts no accelerator runtime (`R_ACCELERATOR_NOT_ADOPTED`; the 0.x facade is such a path). | `backend`, `missing` |
