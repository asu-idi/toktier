...
- `EXPERIMENTAL` is opt-in **only** through an explicit construction
  parameter (`policy=RoutingPolicy.EXPERIMENTAL`); no environment
  variable or config-file value can select it. Streams produced under
  `EXPERIMENTAL` never enter a persistent session store: experimental
  sessions run in-memory only, so an uncertified stream can never be
  replayed into a certified configuration.
...
| `gpu` | CUDA kernel backend (JIT delivery in the first release). |
...
| `R_ACCELERATOR_NOT_ADOPTED` | Device enumeration was not performed: the integrating caller supplied no device probe, i.e. it adopts no accelerator runtime on this path (the 0.x facade is such a path). Says nothing about the machine's hardware. |
...
| `R_INPUT_GUARD_ROUTED` | Reserved for the guarded fast-CPU backend (not shipped in the first release); an input matched a guard predicate and was routed to the reference backend. |
...
routing (`R_INPUT_ADDED_TOKEN`) and correctness fallbacks
